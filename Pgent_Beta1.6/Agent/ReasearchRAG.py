...
# 配置项
deepseek_key= r"sk-4af6"
TXT_DOC_PATH = "./paper_knowledge"
...
def init_rag_engine() -> None:
    ...
    Settings.embed_model = HuggingFaceEmbedding(model_name="/mnt/data/models/all-roberta-large-v1")
    ...
    try:
        if os.path.exists(INDEX_STORAGE_PATH) and os.path.exists(FAISS_INDEX_PATH):
            ...
            text_splitter = SentenceSplitter(chunk_size=512, chunk_overlap=16, separator="<sep>")
...
@tool("research_document_rag", description="用于回答与学术研究内容相关的所有问题，支持总结文档、提取信息、解答文档中的具体问题，返回JSON格式结果")
def research_document_rag(query: str) -> str:
    """LangChain Tool函数：加速版RAG查询"""
